...
get_city <- function(address){
  poss_city <- str_match_all(address, "Nice")[,1]
  poss_city[1]
}
  
get_city(data$all_adress[41])
data$ville <- sapply(data$all_adress, get_city, USE.NAMES = F)

data %>% distinct(all_address)
...
```{r}
new_data <- data %>% select(-Kilométrage,-Puissance.fiscale,-Puissance.réelle,-Garantie.mécanique,-Option.equipement.Sécurité,-Option.Equipement.Confort.intérieur,-Option.equipement.extérieurs,-V1,-Nb.de.portes,-Nombre.de.places,-Consommation.mixte,-Emissions.de.CO2,-Prix,-Autres.informations,-Année)
names(new_data)
```
