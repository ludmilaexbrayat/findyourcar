...
data$postal_code_city <- tolower(gsub("-","",gsub(" ","",str_c(data$postal_codes,data$all_city))))
data$postal_code_city <- sapply(data$postal_code_city, unaccent, USE.NAMES = F)
data %>% distinct(postal_code_city)

relevant_mapping_villes$codes_postaux_commune <- tolower(gsub("-","",gsub(" ","", str_c(relevant_mapping_villes$codes_postaux,relevant_mapping_villes$nom_commune))))
...
data %>% filter(is.na(code_région)) %>% select(postal_code_city) %>% distinct()
