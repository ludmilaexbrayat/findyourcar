...
data %>% 
  ...
  mutate(
         ...
         date = map(Ann??e, clean_month_year),
         pneus_hiver = str_detect(Autres.informations, "Pneus hiver")*1,
         barres_de_toit = str_detect(Autres.informations, "Barres de toit")*1,
         premiere_main = str_detect(Autres.informations, "Premi??re main")*1,
         non_fumeur = str_detect(Autres.informations, "V??hicule non fumeur")*1,
         ...
         kilometrage_km = str_remove(str_remove(Kilom??trage, "\nkm")," "),
         ...
         puissance_reelle_ch = str_remove(Puissance.r??elle, "\nCh"),
         garantie_mecanique_mois = str_remove(Garantie.m??canique, " mois"),
         abs = str_detect(Option.equipement.S??curit??, "ABS")*1,
         esp = str_detect(Option.equipement.S??curit??, "ESP")*1,
         regulateur_vitesse = str_detect(Option.equipement.S??curit??, "R??gulateur de vitesse")*1,
         asr = str_detect(Option.equipement.S??curit??, "ASR")*1,
         alarme_antivol = str_detect(Option.equipement.S??curit??, "Alarme antivol")*1,
         direction_assistee = str_detect(Option.Equipement.Confort.int??rieur, "Direction assist??e")*1,
         banquette_ar = str_detect(Option.Equipement.Confort.int??rieur, "Banquette AR 1/3 - 2/3")*1,
         radio_cd = str_detect(Option.Equipement.Confort.int??rieur, "Radio/CD")*1,
         fermeture_centralisee = str_detect(Option.Equipement.Confort.int??rieur, "Fermeture centralis??e")*1,
         vitres_electriques = str_detect(Option.Equipement.Confort.int??rieur, "Vitres ??lectriques")*1,
         allumage_auto_feux = str_detect(Option.equipement.ext??rieurs, "Allumage automatique des feux")*1,
         peinture_metallisee = str_detect(Option.equipement.ext??rieurs, "Peinture m??tallis??e")*1,
         retroviseurs_electriques = str_detect(Option.equipement.ext??rieurs, "R??troviseurs ??lectriques")*1,
         roues_alliage_l??ger = str_detect(Option.equipement.ext??rieurs, "Roues alliage l??ger")*1,
         fap = str_detect(Option.equipement.ext??rieurs, "FAP")*1
         ) %>% 
  ...
  rename(carrosserie = Carrosserie,
         energie = Energie,
         etat_vehicule = Etat.du.v??hicule,
         transmission = Transmission,
         controle_technique = Contr??le.technique,
         couleur_interieur = Couleur.int??rieur,
  ...
  dplyr::select(-Kilom??trage,
         -Puissance.fiscale,
         -Puissance.r??elle,
         -Garantie.m??canique,
         -Option.equipement.S??curit??,
         -Option.Equipement.Confort.int??rieur,
         -Option.equipement.ext??rieurs,
         -Nb.de.portes,
         -Nombre.de.places,
         -Consommation.mixte,
         -Emissions.de.CO2,
