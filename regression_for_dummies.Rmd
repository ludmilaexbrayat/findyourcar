...
df_significant_coef <- tibble::rownames_to_column(as.data.frame(regression_function(reg_data, !!carrosserie)$coef), "coef")#Create a data frame with no row naes as it is easier to work without
...
negativ_coef <- negativ_coef %>% pmap_chr(.f= function(Estimate,coef){paste("Buying a",coef,"you could save about", -signif(Estimate,2), "€")})

positiv_coef <- positiv_coef %>% pmap_chr(.f= function(Estimate,coef){paste("With a", coef, "you would pay an extra", signif(Estimate,2), "€")})
