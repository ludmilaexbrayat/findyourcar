...
# Loop 1: 1 to 2,000
list_links_1 <- list_searchengine_urls[1:2000] %>% 
...
# Loop 2: 2,001 to 4,000
list_links_2 <- list_searchengine_urls[2001:4000] %>% 
  lapply(scrap_searchengine) %>% 
  unlist(use.names = FALSE)
...
# Loop 3: 4,001 to 6,000
list_links_3 <- list_searchengine_urls[4001/6000] %>% 
  lapply(scrap_searchengine) %>% 
  unlist(use.names = FALSE)
...
# Loop 4: 6,001 to 8,000
list_links_4 <- list_searchengine_urls[6001/8000] %>% 
  lapply(scrap_searchengine) %>% 
  unlist(use.names = FALSE)
...
# Loop 5: 8,001 to 10,000
list_links_5 <- list_searchengine_urls[8001/10000] %>% 
  lapply(scrap_searchengine) %>% 
  unlist(use.names = FALSE)
...
# Loop 6: 10,001 to 10,900
list_links_6 <- list_searchengine_urls[10001/10900] %>% 
  lapply(scrap_searchengine) %>% 
  unlist(use.names = FALSE)
...
xpath_brand <- '(//*[@itemprop="name"])[2]'
xpath_modele <- '(//*[@itemprop="name"])[3]'
node_address <- '.contact_infospart'
node_tableinfo <- 'table.tabcar15'


# Defining a function that scraps a URL
scrap_listing <- function(url) {
  ...
  return(data.frame("url" = paste0(url, ""), ## If we don't add "", and if the variable is empty, we have an error
             "brand" = paste0(brand, ""),
...
scrapped_listings <- rbind.fill(lapply(list_links[1:1000], scrap_listing))
